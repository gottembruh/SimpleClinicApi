...
namespace SimpleClinicApi.DataAccess.Repositories
{
   public class VisitRepository(ClinicDbContext context)
      : GenericRepository<Visit, ClinicDbContext>(context), IVisitRepository
   {
      public async Task<ILookup<Patient, Visit>> GetPatientsToVisitsLookupAsync(
         CancellationToken cancellationToken = default)
      {
         var visitsWithPatients = await _context.Visits
                                                .Include(v => v.Patient)
      ...
      public async Task<Visit?> GetVisitWithDetailsAsync(Guid visitId, CancellationToken cancellationToken)
      {
      ...
      public Task AddVisitProceduresRangeAsync(IEnumerable<VisitProcedure> items, CancellationToken ct)
      {
         return _context.VisitProcedures.AddRangeAsync(items, ct);
      }

      public Task AddVisitMedicationsRangeAsync(IEnumerable<VisitMedication> items, CancellationToken ct)
      {
         return _context.VisitMedications.AddRangeAsync(items, ct);
      }
      ...
      public void RemoveVisitProcedures(Visit visit, IEnumerable<VisitProcedure> proceduresToKeep)
      {
         ...
         if (toRemove.Count != 0)
         {
            _context.VisitProcedures.RemoveRange(toRemove);
         }
      }

      public async Task AddOrUpdateVisitProceduresAsync(Visit visit, IEnumerable<VisitProcedure> procedures,
                                                        CancellationToken cancellationToken)
      {
         var existing = visit.VisitProcedures;

         var toUpdate = existing.Where(ep => procedures.Any(p => p.Id == ep.Id)).ToList();
         IEnumerable<VisitProcedure> visitProcedures = procedures as VisitProcedure[] ?? procedures.ToArray();
         ...
         if (toAdd.Count != 0)
         {
            await _context.VisitProcedures.AddRangeAsync(toAdd, cancellationToken);
         }
      }
      ...
      public void RemoveVisitMedications(Visit visit, IEnumerable<VisitMedication> medicationsToKeep)
      {
         ...
         if (toRemove.Count != 0)
         {
            _context.VisitMedications.RemoveRange(toRemove);
         }
      }

      public async Task AddOrUpdateVisitMedicationsAsync(Visit visit, IEnumerable<VisitMedication> medications,
                                                         CancellationToken cancellationToken)
      {
         var existing = visit.VisitMedications.ToList();

         var toUpdate = existing.Where(em => medications.Any(m => m.Id == em.Id)).ToList();
         IEnumerable<VisitMedication> visitMedications = medications as VisitMedication[] ?? medications.ToArray();
         ...
         if (toAdd.Count != 0)
         {
            await _context.VisitMedications.AddRangeAsync(toAdd, cancellationToken);
         }
      }
   }
}